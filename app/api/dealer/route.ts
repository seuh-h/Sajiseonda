...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === "question") {
      systemMessage = "너는 수평사고 퀴즈 게임의 딜러야. 주어진 규칙에 따라 정해진 4가지 답변 중 하나만 출력해. 절대로 다른 말을 추가하지 마.";
      userPrompt = `너는 수평사고 퀴즈의 딜러야. 아래 사건의 진실을 완전히 이해하고, 플레이어의 질문을 엄격하게 판단해서 4가지 중 하나만 출력해.
...
- 중요하지 않습니다.`;
      ...
      systemMessage = "너는 수평사고 퀴즈 게임의 채점관이야. 반드시 JSON 형식으로만 답해. 다른 텍스트는 절대 출력하지 마.";
      userPrompt = `당신은 수평사고 퀴즈의 딜러입니다.
다음은 이 사건의 완전한 진실입니다:
${solution}
...
keywords 배열은 반드시 메인 키워드 목록의 모든 항목을 포함해야 합니다. 빠뜨리지 마세요.`;
    ...
    const model = type === "answer" ? GEMINI_MODEL_ANSWER : GEMINI_MODEL_QUESTION;
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Gemini API error: ${response.status}`, detail: errBody }, { status: 500 });
    }
    ...
    console.error("[dealer route error]", err);
